        ...
        private static readonly IDictionary<string, string> _operators = new Dictionary<string, string>
    {
        ...
        public static IQueryable<T> ToDynamic<T>(this IQueryable<T> query, DynamicQuery dynamicQuery)
        {
            if (dynamicQuery.Filter is not null)
                query = Filter(query, dynamicQuery.Filter); //filtreyi ekle sorguya
            if (dynamicQuery.Sort is not null && dynamicQuery.Sort.Any())
                query = Sort(query, dynamicQuery.Sort); //sıralamayı ekle sorguya
            return query;
        }
        ...
        {
            filters.Add(filter);
            if (filter.Filters is not null && filter.Filters.Any()) //filtrenin alt filtreleri varsa onları da ekle
        ...
        public static string Transform(Filter filter, IList<Filter> filters)
        {
            ...
            if (!string.IsNullOrEmpty(filter.Value))
            {
                if (filter.Operator == "doesnotcontain")
                    where.Append($"(!np({filter.Field}).{comparison}(@{index.ToString()}))"); //np:null propagation: null verileri ne yapayımın ynaıtını veri. Al ya da alma şeklinde
